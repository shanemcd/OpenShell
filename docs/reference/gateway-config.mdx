...
# When false, omit the default workspace PVC + /sandbox mount (Pod and VirtualMachine).
# workspace_persistence          = true
# Emit VirtualMachine Sandbox CRs for KubeVirt via agent-sandbox (empty or "Pod" = Pods).
# runtime_backend                = "VirtualMachine"
# Default OPENSHELL_SANDBOX_COMMAND for VirtualMachine sandboxes.
