...
| `workspace_persistence` | `server.workspacePersistence` | When true (default), inject a workspace PVC mounted at `/sandbox` for Pod and VirtualMachine backends. |
| `runtime_backend` | `server.runtimeBackend` | Set to `VirtualMachine` so agent-sandbox creates a KubeVirt VM instead of a Pod. Empty or `Pod` keeps the Pod path. |
| `sandbox_command` | `server.sandboxCommand` | Default `OPENSHELL_SANDBOX_COMMAND` for VirtualMachine sandboxes (for example `/usr/local/bin/nemoclaw-start-vm`). |
