...
| `workspace_default_storage_size` | `server.workspaceDefaultStorageSize` | Set the default workspace PVC size for new sandboxes (Pod and VirtualMachine). |
| `workspace_persistence` | `server.workspacePersistence` | When true (default), emit a workspace PVC mounted at `/sandbox`. Set false to omit the claim for both backends. |
