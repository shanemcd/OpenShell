...
pub async fn run_sandbox(
    ...
    let exit_code = if process_enabled {
        ...
        // workload (KubeVirt/systemd), adopt its entrypoint PID for proxy
        // identity binding, and keep the proxy alive until shutdown. If an
        // authoritative process-supervisor control connection closes, exit
        // non-zero so Kubernetes restarts the network sidecar.
        #[cfg(target_os = "linux")]
        {
            if let Some(ns) = netns.as_ref() {
                if let Err(err) =
